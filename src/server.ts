...
app.get("/courses/:id/modules", async (request: Request, response: Response) => {
    const courses = await knexConfig("courses")
        .select("courses.id AS course_id", "course_modules.id AS module_id", "course_modules.name AS module", "courses.name AS courses")
        .join("course_modules", "id_courses", "course_modules.id_courses")
